# 🚀 Test-Ausführung: Live-Logs anzeigen
...
### Option 1: Terminal mit Live-Logs (Empfohlen)
...
### Option 2: Mit Live-Log-Skript

```bash
python scripts/validation/run_strategy_validation_with_logs.py
```
...
[2025-11-06 12:00:15 - INFO] Phase 2: Simple P&ID analysis (Monolith → Swarm as hint list)...
...
Die Logs werden **live** im Terminal angezeigt, sodass Sie:

1. **Fortschritt sehen:** Welche Phase läuft gerade?
2. **Fehler erkennen:** Gibt es Probleme?
3. **Performance beobachten:** Wie lange dauert jede Phase?
4. **Ergebnisse sehen:** F1-Scores werden sofort angezeigt

---

## 💡 Tipps

### 1. Starten Sie mit Test 2

**Test 2** ist der schnellste und einfachste Test (~1-2 Minuten):
```bash
python scripts/validation/run_strategy_validation.py --test "Test 2"
```
...
- **Phase 0-1:** Sollten schnell sein (~10-20 Sekunden)
- **Phase 2:** Dauert am längsten (30-120 Sekunden je nach Test)
- **Phase 4:** Sollte schnell sein (~10-20 Sekunden)

### 3. Bei Fehlern

- **GCP_PROJECT_ID nicht gesetzt:** Setzen Sie die Umgebungsvariablen
- **Bild nicht gefunden:** Prüfen Sie den Pfad in `--image`
- **Ground Truth nicht gefunden:** Test läuft trotzdem (ohne F1-Scores)
...
1. **Ergebnisse analysieren:** `outputs/strategy_validation/summary_*.json`
2. **Parameter anpassen:** Basierend auf Analyse-Ergebnissen
3. **Training Camp (optional):** Automatische Parameter-Optimierung
4. **Validierung:** Tests erneut ausführen
